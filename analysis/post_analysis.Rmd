...
```{r, data_cleaning, cache = TRUE, cache.lazy = FALSE}
...
vals <- vals %>% 
  ...
  rename(uSimp.locus = uSimp.y) -> vals
...
```{r, quality_control}
runs    <- as.character(1:16)
tens    <- runs[9:16]          # The last 8 runs had 9 and 10 seeds, alternating
nines   <- tens[1:8 %% 2 == 1] # If I ever do this again, tell me no. :/
tens    <- tens[1:8 %% 2 == 0]
sexrate <- c("0.0000", "1.0000", "0.0001", "0.0005", "0.0010", "0.0050", 
             "0.0100", "0.0500", "0.1000", "0.5000")
ten_seed <- expand.grid("twenty_loci", tens, 
            "/seed_", as.character(0:9), # ten seeds
            "_sex_", sexrate,
            "_gen_10000", 
            "_rep_", paste0("0", as.character(0:9)),
            ".pop_sam_", as.character(c(10, 25, 50, 100))) %>% 
  apply(1, paste, collapse = "")
nine_seed <- expand.grid("twenty_loci", nines, 
            "/seed_", as.character(0:8), # nine seeds
            "_sex_", sexrate,
            "_gen_10000", 
            "_rep_", paste0("0", as.character(0:9)),
            ".pop_sam_", as.character(c(10, 25, 50, 100))) %>% 
  apply(1, paste, collapse = "")
three_seed <- expand.grid("twenty_loci", runs[1:8], # The first eight runs
            "/seed_", as.character(0:2),            # had three seeds.
            "_sex_", sexrate,
            "_gen_10000", 
            "_rep_", paste0("0", as.character(0:9)),
            ".pop_sam_", as.character(c(10, 25, 50, 100))) %>% 
  apply(1, paste, collapse = "")
expected <- sort(c(ten_seed, nine_seed, three_seed)) # Expect 40,000

# Extracting the unique seeds
mismatches <- !expected %in% (vals %>% arrange(pop))$pop
strsplit(expected[mismatches], "_sam") %>%
  vapply("[", character(1), 1) %>%
  unique() %>%
  cat(sep = "\n", file = "MISSING_DATASETS.txt")
...
```{r, rd_pval_cc}
ggplot(vals, aes(x = p.rDcc, y = p.rD)) +
  geom_point() +
  theme_bw() +
  geom_abline(slope = 1) +
