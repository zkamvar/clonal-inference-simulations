...
rd_sexrate <- ggplot(vals %>% select(sexrate, rbarD, sample, mutation_rate), 
...
rd_pval <- ggplot(vals %>% select(sexrate, p.rD, sample, mutation_rate), 
...
rd_pval_cc <- ggplot(vals %>% select(p.rDcc, p.rD, mutation_rate, sample, sexrate), 
       aes(x = p.rDcc, y = p.rD, pch = mutation_rate)) +
