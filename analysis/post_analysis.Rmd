...
```{r}
ggplot(filter(even_mutation, p.rD >= 0.05), aes(x = Evennesscc, y = rbarD, color = log(NMLG))) +
  geom_point(alpha = 0.25) +
...
ggplot(filter(odd_mutation, p.rD >= 0.05), aes(x = Evennesscc, y = rbarD, color = log(NMLG))) +
  geom_point(alpha = 0.25) +
  scale_color_viridis(option = "magma", direction = -1,
                      breaks = log(c(2, 5, 10, 25, 50, 100)),
                      labels = c(2, 5, 10, 25, 50, 100)) +
