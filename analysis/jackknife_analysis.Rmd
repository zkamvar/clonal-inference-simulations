...
Bias[\hat\theta] = \hat\theta - \hat\theta_B
$$

Once I have the bias, I'm going to compare the bias-corrected value of the
clone-corrected $\bar{r}_d$ to bootstrapped distribution, which would tell us
how many samples provided a value of $\bar{r}_d$ less than the clone-corrected
value (good example from <http://www.stat.umn.edu/geyer/5601/examp/bias.html>).
...
bvals <- vals %>%
  ...
  mutate(bias = rbarD - jack.mean) %>% # calculating bias a la Effron
  select(bias, jack.mean, everything()) %>%
  mutate(jack.p.rDb = map2_dbl(rbarDcc - bias, jack.rd, makep)) %>%
...
bvals %>%
  ...
  spread(sample, reject) %>%
  knitr::kable(digits = 3, caption = "rejected fraction (median bias)")
```
...
bvals %>%
  ...
  geom_density(alpha = 0.5, binwidth = 0.05) +
  geom_rug(alpha = 0.5) +
  facet_grid(sexrate~sample, scales = "free_y") +
  xlab("Fraction of bootstraps <= clone-corrected")
...
equal to the clone-corrected size, we can obtain a distribution that shows us
how much we expect the effect of a reduced sample size has. We can then 
correct the clone-corrected value of $\bar{r}_d$ for the bias due to sample size
and compare that to the expected values.
