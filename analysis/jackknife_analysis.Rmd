...
Here, I'm testing to see what happens when we "correct" for this bias by
...
bvals <- vals %>%
  ...
  mutate(bias = rbarD - jack.mean) %>%
  select(bias, jack.mean, everything()) %>%
  mutate(jack.rdb = map2(jack.rd, bias, `+`)) %>%
  mutate(jack.p.rDb = map2_dbl(rbarDcc, jack.rdb, makep)) %>%
  select(bias, jack.p.rD, jack.p.rDb, everything())
bvals %>%
  group_by(sexrate, sample) %>%
  summarize(reject = sprintf("%.2f (%.4f)", sum(jack.p.rDb < 0.05)/n(), median(bias, na.rm = TRUE))) %>%
  spread(sample, reject)
bvals %>%
