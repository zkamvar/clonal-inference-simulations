...
datfiles <- dir("simulation_analysis/rda_files/", full.names = TRUE) %>% grep("feather.rda$", ., value = TRUE)
...
ggplot(vals, aes(x = sexrate, y = rbarD, color = log(p.rD))) +
  ...
  scale_color_viridis(option = "magma", breaks = log(c(0.005, 0.01, 0.025, 0.05, 0.1)), labels = c(0.005, 0.01, 0.025, 0.05, 0.1)) +
  facet_wrap(~sample)
