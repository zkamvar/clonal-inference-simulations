...
"
================================================================================
Parse RDA files containing genclone objects, analyze clonecorrected IA vs.
populations subsampled to the number of multilocus genotypes. A tibble is
returned containing the fraction of jackknife data sets with a value of IA less
...
for (f in opt$FILE){
  ...
  if (opt$mutant){
    moutf <- gsub(".DATA", ".jackia.mutant", basename(f))
    assign(x = moutf, mres)
