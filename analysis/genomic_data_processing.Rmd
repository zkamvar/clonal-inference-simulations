...
datnames <- vector(mode = "list", length = length(datfiles)) %>% setNames(datfiles)
for (i in datfiles){
  datnames[[i]] <- load(i)
}

datnames <- unlist(datnames, use.names = FALSE)
...
shufffiles <- vector(mode = "list", length = length(datfiles)) %>% setNames(datfiles)
for (i in datfiles){
  shufffiles[[i]] <- load(i)
}

shufffiles <- unlist(shufffiles, use.names = FALSE)
...
(n        <- 5) # This givest the magic number!
...
## High recombination data

After analysis, the original data showed so much linkage that we decided to
virtually expand the the chromosomes by setting the recombination rate between
adjacent sites to 1e-3. This means that there is a probability of 1
recombination event on a chromosome with 1000 sites. I will refer to these data
as **hr** data for "high recombination" data. 


```{r, load_hr_data, results = "hide"}
# Here we have to filter out the stored genetic data
datfiles <- dir("../../simulation_results/genomic_hr_rda_files/", full.names = TRUE) %>% 
  grep("feather.rda$", ., value = TRUE)

# The HR data has a different prefix for the directory.
hr_run  <- "genomic_high_recombination([0-9]+?)/"
...
datalist <- datnames %>%
  ...
  map_df(get, .id = "source") %>% # map the `get()` function and return a tibble
  extract(pop, c("run", "seed", "sexrate", "gen", "rep", "sample"),
          paste0(hr_run, ex_seed, ex_sex, ex_gen, ex_rep, ex_samp),
          remove = FALSE) %>%     # process the metadata
  mutate(sample = factor(sample)) %>%     # Transform the sample to a factor
  select(missing_0.01:sample, source) %>% # resort the columns
  distinct(run, seed, sexrate, rep, sample, .keep_all = TRUE) # filter out duplicates
...
hr_missing_stats <- filter(datalist, matching != "none") %>% # data set for only rd from missing data
  select(-samples, -p.rD)
hr_original        <- filter(datalist, matching == "none") %>% 
  select(-matching, -missing) # rd with no missing data
rm(list = datnames)
gc(verbose = TRUE)
hr_original
...
Since analyzing the index with any linkage will cloud the result, we present 
here the shuffle-by-chromosome method. At this moment, since I don't have all
the data available, I am not filtering for different data between 10000 and
10001 generations. 

```{r, load_hr_redux, restulst = "hide"}
# Here we have to filter out the stored genetic data
datfiles <- dir("../../simulation_results/genomic_hr_rda_files_redux/", full.names = TRUE)
...
```{r}
save(original_redux, original, missing_stats, 
     hr_original_redux, hr_original, hr_missing_stats, 
     file = "../../simulation_results/genomic_data.rda")
```
