...
ggAURC_ANOVA <- ggplot(AURC_ANOVA_III %>% filter_anova, aes(x = sexrate, y = forcats::fct_rev(term), fill = p.value)) + 
  geom_tile() + 
...
overall_roc_table <- roc_overall %>% 
  ...
  mutate(clone_correction = forcats::fct_rev(clone_correction)) %>%
  fancy_mutation_rate
powerplot0.01 <- overall_roc_table %>% 
  filter(alpha == 0.01) %>%
  ...
  facet_grid(clone_correction ~ mutation_rate) +
  ...
  sample_colors
powerplot0.05 <- overall_roc_table %>% 
  filter(alpha == 0.05) %>%
  ggplot(aes(y = `True Positive`, x = sexrate, color = sample, alpha = `False Positive`)) +
  geom_point(size = 2) + 
  # geom_errorbar(aes(ymax = `True Positive` + TPsd, ymin = `True Positive` - TPsd), width = 0.25) +
