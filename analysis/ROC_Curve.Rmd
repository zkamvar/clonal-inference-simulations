...
```{r}
by_seed %>%
  group_by(sexrate, sample, mutation_rate, CC) %>%
  do(shapiro.test(.$AURC) %>% tidy()) %>%
  ungroup() %>% 
  summarize(percent_normal = sum(p.value >= 0.1)/n())
car::leveneTest(AURC ~ factor(sexrate) * sample * mutation_rate * CC, data = by_seed)
