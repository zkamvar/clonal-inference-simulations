...
ggsave(plot = oroc_plot_mod, 
       file = "~/Documents/Grunwald/dissertation/figure/simulations/ROC_Curve.pdf",
...
ggsave(plot = AURC_box_plot, width = 6, height = 7, dpi = 600,
       file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_box_plot.pdf")
ggsave(plot = AURC_box_plot_ea, width = 6, height = 7, dpi = 600,
       file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_box_plot_ea.pdf")
...
grid.newpage()
grid.draw(g)
# ggsave(plot = g, file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_ANOVA.pdf",
...
```{r full_anova, fig.width = 8, fig.height = 5}
clean_anova <- . %>%
...
ggsave(plot = ggAURC_ANOVA, file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_ANOVA.pdf",
       width = 8, height = 5, dpi = 600)
ggsave(plot = ggAURC_ANOVA_ea, file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_ANOVA_ea.pdf",
       width = 8, height = 5, dpi = 600)
ggAURC_ANOVA
ggAURC_ANOVA_ea
...
ggsave(plot = powerplot0.01, file = "~/Documents/Grunwald/dissertation/figure/simulations/ssr_power.pdf",
       width = 8, height = 5, dpi = 600)
ggsave(plot = powerplot0.01_ea, file = "~/Documents/Grunwald/dissertation/figure/simulations/ssr_power_ea.pdf",
       width = 8, height = 5, dpi = 600)
...
g_oroc_plot <- ggplot(g_roc_overall, aes(x = `False Positive`, y = `True Positive`)) +
  ...
  sample_fill
g_oroc_plot
hr_g_oroc_plot <- ggplot(hr_g_roc_overall, aes(x = `False Positive`, y = `True Positive`)) +
  geom_ribbon(aes(ymin = `True Positive` - TPsd, ymax = `True Positive` + TPsd, fill = sample), 
              alpha = 0.25) +
  geom_line(aes(color = sample)) +
  facet_wrap(~sexrate) +
...
g_total_oroc_plot <- ggplot(g_total_roc, aes(x = `False Positive`, y = `True Positive`)) +
  ...
  sample_fill
g_total_oroc_plot
hr_g_total_oroc_plot <- ggplot(hr_g_total_roc, aes(x = `False Positive`, y = `True Positive`)) +
  geom_ribbon(aes(ymin = `True Positive` - TPsd, ymax = `True Positive` + TPsd, fill = sample), 
              alpha = 0.25) +
  geom_line(aes(color = sample)) +
  geom_abline(slope = 1, lty = 3) +
...
g_AURC_by_seed_plot <- ggplot(g_AURC_by_seed, aes(x = factor(sexrate), y = AURC)) +
  ...
  theme(strip.text = element_text(size = 14, face = "bold")) +
  theme(aspect.ratio = 0.5)
hr_g_AURC_by_seed_plot <- ggplot(hr_g_AURC_by_seed, aes(x = factor(sexrate), y = AURC)) +
  geom_point(position = position_jitter(height = 0, width = 0.125), alpha = 0.5) +
  annotate("rect", xmin = 0, xmax = 9.75, ymin = 0, ymax = 0.5, alpha = 0.2) +
  facet_wrap(~sample) +
  geom_text(aes(x = x, y = y, label = text), hjust = -0.1, vjust = 1.5, color = "gray25",
...
ggsave(plot = g_AURC_by_seed_plot, file = "~/Documents/Grunwald/dissertation/figure/simulations/AURC_genomic.pdf",
       width = 6, height = 4, dpi = 600)
g_AURC_by_seed_plot
hr_g_AURC_by_seed_plot
```


```{r genomic_power, fig.width = 4.25, fig.height = 5}
...
powerplot <- g_overall_roc_table %>% 
  ...
  scale_alpha(range = c(1, 0.5), breaks = c(0.01, 0.03, 0.05)) +
  theme(aspect.ratio = 0.5) +
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_blank()) +
  theme(text = element_text(size = 14)) +
  ...
  sample_colors
hr_g_overall_roc_table <- hr_g_roc_overall %>% 
  filter(alpha %in% c(0.01, 0.05)) %>%
  mutate(alpha = paste("p =", alpha))
hr_powerplot <- hr_g_overall_roc_table %>% 
  ggplot(aes(y = `True Positive`, x = sexrate, color = sample, alpha = `False Positive`)) +
  geom_point(size = 2) + 
  # geom_errorbar(aes(ymax = `True Positive` + TPsd, ymin = `True Positive` - TPsd), width = 0.25) +
  geom_line(aes(group = sample)) +
  facet_wrap(~alpha, ncol = 1) +
  scale_alpha(range = c(0.5, 0.05)) +
  theme(aspect.ratio = 0.5) +
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_blank()) +
  theme(text = element_text(size = 14)) +
  ...
  sample_colors
ggsave(file = "~/Documents/Grunwald/dissertation/figure/simulations/genomic_power.pdf",
       plot = powerplot, width = 4.5, height = 5, dpi = 600)
powerplot
hr_powerplot
```
