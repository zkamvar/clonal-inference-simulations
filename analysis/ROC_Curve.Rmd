...
process_roc <- . %>%
  ...
  mutate(sdtp = roc_sd(`True Positive`, ntp), sdfp = roc_sd(`False Positive`, nfp)) %>%
  mutate(ymax = `True Positive` + sdtp, ymin = `True Positive` - sdtp,
         xmax = `False Positive` + sdfp, xmin = `False Positive` - sdfp)
```
...
total_roc <- map_df(alpha, roc, vals %>% mutate(sexrate = ifelse(sexrate == "1.0000", "R", "NR")),
...
total_roc_cc <- map_df(alpha, roc, vals %>% mutate(sexrate = ifelse(sexrate == "1.0000", "R", "NR")),
                      compare = c("NR", "R"), stat = "p.rDcc", count.na = TRUE,
                      group = c("mutation_rate", "sexrate", "sample")) %>%
  process_roc
total_roc_cc

total_roc <- bind_rows(uncorrected = total_roc, `clone-corrected` = total_roc_cc, .id = "Clone Correction")
OVERALL_ROC <- bind_rows(uncorrected = roc_overall, `clone-corrected` = roc_overall_cc, .id = "Clone Correction") %>% select(-ntp, -nfp, -sdtp, -sdfp, -xmax, -xmin, -sex)
...
oroc_plot <- ggplot(OVERALL_ROC, aes(x = `False Positive`, y = `True Positive`)) +
  geom_line(aes(color = sample)) +
  facet_grid(sexrate~mutation_rate+`Clone Correction`) +
...
total_oroc_plot <- ggplot(total_roc, aes(x = `False Positive`, y = `True Positive`)) +
  geom_line(aes(color = sample)) +
  facet_grid(mutation_rate~`Clone Correction`) +
  geom_abline(slope = 1, lty = 3) +
  labs(list(
    title = "ROC Curves",
    pch = "Sample Size",
