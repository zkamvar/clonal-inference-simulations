...
gtable_add_grob(res, z$grobs[[locations[3]]]$grobs[[1]], 1, 1, 1, 3) %>%
...
AURC_box_plot_ea <- AURC_by_seed_ea %>%
  fancy_clone_correction %>%
  mutate(clone_correction = forcats::fct_rev(clone_correction)) %>%
  fancy_mutation_rate %>%
  ggplot(aes(x = factor(sexrate), y = AURC)) +
...
ggAURC_ANOVA_ea <- AURC_ANOVA_ea_III %>% 
  filter_anova %>%
  ggplot(aes(x = sexrate, y = forcats::fct_rev(term), fill = p.value)) + 
  geom_tile() + 
  geom_text(aes(label = round(p.value, 2), color = 1 - p.value)) +
...
```{r genomic_power, fig.width = 6, fig.height = 5}
...
ggsave(file = "~/Documents/Grunwald/dissertation/figure/simulations/genomic_power.png",
       plot = powerplot, width = 6, height = 5, dpi = 600)
powerplot
hr_powerplot
```
