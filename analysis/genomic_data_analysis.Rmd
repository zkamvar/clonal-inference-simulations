...
datalist <- datalist  %>%
  ...
  mutate(sample = forcats::fct_relevel(sample, 10, 25, 50))
...
shuffs <- shufffiles %>% 
  ...
  distinct(run, seed, sexrate, rep, sample, .keep_all = TRUE) %>%
  group_by(run, seed, sexrate, sample) %>%
  sample_n(6)
shuffs
```
...
```{r, corrected_rd}
corrected_rd <- ggplot(shuffs %>% select(sexrate, rbarD, sample), 
       aes(x = sexrate, y = rbarD, fill = sample)) +
  geom_boxplot() +
  scale_fill_grey() +
    labs(list(
    title = "index of association by sex rate",
...
correcte_rd_p <- ggplot(shuffs %>% select(sexrate, p.rD, sample), 
       aes(x = sexrate, y = p.rD, fill = sample)) +
  geom_boxplot() + 
...
Now we can save the results for ROC analysis:


```{r, save_data}
genomic_data <- shuffs
save(genomic_data, file = "../../simulation_results/genomic_data.rda")
```
