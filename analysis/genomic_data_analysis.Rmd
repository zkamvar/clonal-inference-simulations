...
genomic_data <- load(here::here("data", "genomic_data.rda"))
genomic_data
```
...
old_theme <- theme_update(text = element_text(size = 16))
...
missing_plot <- missing_stats %>% 
  ...
  scale_y_continuous(position = "right", breaks = c(0, 0.5, 1), 
                     minor_breaks = c(0.25, 0.75), limits = c(NA, 1)) +
...
if (!interactive()){
  ggsave(filename = here::here("manuscript", "figure", "genomic_missing.pdf"),
       plot = missing_plot, dpi = 600, height = 6, width = 7)
}
...
```{r, load_examples}
load(here::here("data", "genomic_rda_files/X.data.3639601.100.bpp.dir_genomic10_gen_10seed_0_sex_1.0000_gen_10000_rep_09.pop.feather.DATA.rda")) %>% 
  get() %>% 
  tidyr::gather(key = percent_missing, value = dataset, starts_with("dataset")) -> x

xdat <- x$dataset[[2]]
xdat
load(here::here("data", "genomic_rda_files/X.data.3639601.1.grungall1.dir_genomic10_gen_10seed_0_sex_0.0000_gen_10000_rep_01.pop.feather.DATA.rda")) %>%
  get() %>%
...
```{r, datrd}
barplot(c(sexual = bitwise.ia(xdat, threads = 1L), 
          clonal = bitwise.ia(ydat, threads = 1L),
          simulated = bitwise.ia(xsim, threads = 1L)), log = "y", ylab = "log index of association")
```
...
```{r, dendrogrammin}
library('ape')
xdat %>% bitwise.dist(threads = 1L) %>% nj() -> sexdist
ydat %>% bitwise.dist(threads = 1L) %>% nj() -> clonedist
xsim %>% bitwise.dist(threads = 1L) %>% nj() -> simdist
par(mfrow = c(1, 3))
...
xdat %>% shuffle_genlight(mat = as.matrix(.), x = ., blocksize = 1) %>% bitwise.dist(threads = 1L) %>% nj() -> sexdist
ydat %>% shuffle_genlight(mat = as.matrix(.), x = ., blocksize = 1) %>% bitwise.dist(threads = 1L) %>% nj() -> clonedist
xsim %>% shuffle_genlight(mat = as.matrix(.), x = ., blocksize = 1) %>% bitwise.dist(threads = 1L) %>% nj() -> simdist
par(mfrow = c(1, 3))
plot(sexdist, main = "Sexual")
add.scale.bar(x = 0.05, y = -0.1, lcol = "red", length = 0.05)
plot(clonedist, main = "Clonal")
...
if (!interactive()){
  ggsave(filename = here::here("manuscript", "figure", "genomic_rd.pdf"),
