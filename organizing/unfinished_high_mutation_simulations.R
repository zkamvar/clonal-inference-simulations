...
twelve   <- runs[1:8 %% 2 == 0] # If I ever do this again, tell me no. :/
thirteen <- runs[1:8 %% 2 == 1]
sexrate <- c("0.0000", "0.0001")
twelve_seed <- expand.grid("high_mutation", twelve,
                        "/seed_", as.character(0:11), # twelve seeds
                        "_sex_", sexrate,
                        "_gen_10000",
                        "_rep_", paste0("0", as.character(0:9)), ".pop") %>%
  apply(1, paste, collapse = "")
thirteen_seed <- expand.grid("high_mutation", thirteen,
...
dirs <- sapply(paste0("../zhian_ssr_data/high_mutation", 1:8), dir, full.names = TRUE)
...
cat(run, sep = "\n", file = "analysis/high_mutation_rerun.txt")
