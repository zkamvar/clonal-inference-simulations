...
options = [
    ...
    {'name':'R0',
     'default':1,
     'label':'Number of Replicates (n replicates will output 32n files)',
     'description':'Set the number of replicate simulations you wish to perform. IMPORTANT: number of replicates is directly related to the number of files you will produce, so n replicates will produce 1+32n files.',
     'type': 'integer',
     'validator': 'R0 > 0',
    },
    {'name':'L0',
...
%.2f | Rep: %d | Gen: %d\n' % (popSize, numOfMales, heteroFreq[1], rep+"+str(R0)+", gen-("+str(G0)+"/10))", step=G0/10),
            sim.SavePopulation(output="!'"+str(N0)+"/"+str(N0)+
            "_rep_%02d_gen_%d.pop' % (rep+"+str(R0)+",gen-("+str(G0)+"/10))", step=G0/10),
        ],
        finalOps=[
            sim.SavePopulation(output="!'"+str(N0)+"/"+str(N0)+
            "_rep_%02d_gen_%d.pop' % (rep+"+str(R0)+",gen-("+str(G0)+"/10))", step=G0/10),
...
#    commands.getoutput('mkdir '+str(N0))
    ...
    simu.evolve(
        ...
        finalOps = sim.SavePopulation('!"BURNIN_%d.pop" % regul(B0, B0)'),
        gen = B0
    )
...
def partialCloneFromBurnin(Sam0, G0, C0, R0, N0, L0):
    pop = sim.loadPopulation("./BURNIN.pop")
    sample = drawRandomSample(pop, sizes=Sam0)
    commands.getoutput('mkdir '+str(N0))
...
#------------------------------------------------------------------------------#
    simu = sim.Simulator(pop, R0)
    gens = simu.evolve(
        #initOps = sim.SavePopulation(output="!'"+str(N0)+"/"+str(N0)+
        #    "_rep_%02d_gen_0.pop' % (rep+1)"),
        ...
        postOps=[
            ...
            sim.Stat(popSize=True, numOfMales=True, heteroFreq=L0, step=G0/10),
            sim.PyEval(r"'Pop Size: %d | Number of Males: %d | Heterozygosity: \
%.2f | Rep: %d | Gen: %d\n' % (popSize, numOfMales, heteroFreq[1], rep+1, gen-("+str(G0)+"/10))", step=G0/10),
...
#------------------------------------------------------------------------------#
            sim.SavePopulation(output="!'"+str(N0)+"/"+str(N0)+
            "_rep_%02d_gen_%d.pop' % (rep+1,gen-("+str(G0)+"/10))", step=G0/10),
        ],
        finalOps=[
            sim.SavePopulation(output="!'"+str(N0)+"/"+str(N0)+
            "_rep_%02d_gen_%d.pop' % (rep+1,gen-("+str(G0)+"/10))", step=G0/10),
...
#------------------------------------------------------------------------------#
    for k in range(R0):
        for m in range(0,G0+1,(G0)/10):
            ppop = sim.loadPopulation(str(N0)+"/"+str(N0)+"_rep_%02d_gen_%d.pop" % (k+1,m))
...
#------------------------------------------------------------------------------#
    ...
    loclist = pickle.load(open("BURNIN.p", "rb"))
#------------------------------------------------------------------------------#
# The running of the simulation with all the parameters is given by this line.
#------------------------------------------------------------------------------#
    partialCloneFromBurnin(pars.Sam0, pars.G0, pars.C0, pars.R0, pars.N0, pars.L0)
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
    ...
    with open('./LOG_'+str(pars.N0)+'.txt', 'a') as f:
        ...
        f.write(
            ...
            "#\tNumber of Replicates: "+str(pars.R0)+"\n"
